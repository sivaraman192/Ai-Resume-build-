...
const PATH_SEPARATOR_PATTERN = node_path_1.sep === '\\' ? /[\\/]/ : /\//;
const SEPARATOR_DESCRIPTION = node_path_1.sep === '\\' ? '"/" or "\\"' : '"/"';
function assertFlatFilename(name, pageNumber) {
    if (PATH_SEPARATOR_PATTERN.test(name)) {
        throw new Error(`outputFileMaskFunc returned a filename containing a path separator for page ${pageNumber}: "${name}". The filename must be a flat name with no ${SEPARATOR_DESCRIPTION} characters.`);
    }
}
...
async function processAndSavePage(pdfDocument, pageName, pageNumber, pageViewportScale, mode) {
    ...
    const pageOutput = await (0, pageRenderer_js_1.renderPdfPage)(pdfDocument, pageName, pageNumber, pageViewportScale, shouldReturnContent);
